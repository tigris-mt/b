...
-- Returns nil if invalid or unparseable colorspec.
function b.color.convert(c)
	...
	elseif type(c) == "number" then
		return {
			a = math.floor((c / (2 ^ 24)) % 0x100),
			r = math.floor((c / (2 ^ 16)) % 0x100),
			g = math.floor((c / (2 ^ 8)) % 0x100),
	...
	elseif type(c) == "string" then
		...
		ret = ret or try_match("#(%x%x)(%x%x)(%x%x)", function(r, g, b)
			return {r = tonumber(r, 16), g = tonumber(g, 16), b = tonumber(b, 16)}
		end)
		ret = ret or try_match("#(%x)(%x)(%x)(%x)", function(r, g, b, a)
			return {r = tonumber(r .. r, 16), g = tonumber(g .. g, 16), b = tonumber(b .. b, 16), a = tonumber(a .. a, 16)}
		end)
		ret = ret or try_match("#(%x)(%x)(%x)", function(r, g, b)
			return {r = tonumber(r .. r, 16), g = tonumber(g .. g, 16), b = tonumber(b .. b, 16)}
		end)
